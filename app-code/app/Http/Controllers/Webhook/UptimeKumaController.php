...
class UptimeKumaController extends Controller
{
    public function __invoke(Request $request): JsonResponse
    {
        ...
        $site = Site::where('uptime_kuma_monitor_id', (int) $monitorId)->first();
    ...
    private function handleSiteDown(Site $site, string $message): void
    {
        ...
        if ($site->status !== SiteStatus::DOWN) {
            ...
            Event::create([
                ...
                'title'     => "Site offline: {$site->domain}",
                'message'   => $message ?: 'Uptime Kuma detected the site is unreachable.',
                'metadata'  => ['source' => 'uptime_kuma', 'direction' => 'down'],
                'resolved'  => false,
            ]);

            Log::warning('UptimeKuma webhook: site down', ['site' => $site->domain, 'site_id' => $site->id]);
    ...
    private function handleSiteUp(Site $site, string $message): void
    {
        ...
        if ($site->status === SiteStatus::DOWN) {
            ...
            Event::create([
                ...
                'title'     => "Site back online: {$site->domain}",
            ...
            Log::info('UptimeKuma webhook: site recovered', ['site' => $site->domain, 'site_id' => $site->id]);
